...
router.get("/messages", async (req, res) => {
    const mongo = db.getDB();
    let messages = await mongo.collection("messages").find().sort({ _id: -1 }).toArray();
    res.json(messages);
});
...
router.get("/messages/:id", async (req, res) => { 
    ...
    let message = await mongo.collection("messages").find({_id:idMsg});
    res.json(message);
});
...
router.get("/feed", async (req, res) => {
    try {
        ...
        // Convert the set to an array and sort the messages by date in descending order
        const sortedMessages = Array.from(followedMessages).sort((a, b) => new Date(b.date) - new Date(a.date));
